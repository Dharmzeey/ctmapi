...
"""
class FetchLocation(APIView):
  def get(self, request):
    states = State.objects.all()
    location = Location.objects.all()
    institution = Institution.objects.all()
    state_serializer = customAPISerializers.StateSerializer(instance=states, many=True)
    loaction_serializer = customAPISerializers.LocationSerializer(instance=location, many=True)
    institution_serializer = customAPISerializers.InstitutionSerializer(instance=institution, many=True)
    data = {
      "states": state_serializer.data,
      "locations": loaction_serializer.data,
      "institutions": institution_serializer.data,
    }
    return Response(data, status=status.HTTP_200_OK)
fetch_location = FetchLocation.as_view()

class HomePage(APIView):
  def get(self, request):
    product = Product.objects.filter(vendor__active_subscription=True)
    product_serializer = customAPISerializers.ProductSerializer(instance=product, many=True)
    data = {
